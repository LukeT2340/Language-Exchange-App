...
struct LoginMenuView: View {
    ...
    var body: some View {
        VStack (spacing: 15) {
            ...
            Button(action: {
                ...
                if !authManager.isLoggingIn {
                    Text(NSLocalizedString("Login-Button", comment: "Login button"))
                        .fontWeight(.medium)
                        .buttonStyle()
                } else {
                    ProgressView()
            ...
            if !keyboardResponder.isVisible {
                ...
                SignInWithAppleButton(
                       ...
                       onRequest: { request in
                           authManager.signInWithAppleRequest(request)
                       },
                       onCompletion: { result in
                           authManager.signInWithAppleCompletion(result)
                       }
                   )
        ...
        .navigationTitle(LocalizedStringKey("Login"))
        .onTapGesture {
            self.hideKeyboard()
        }
